/*per desk score record model class
 * 
 * author shi
 * 
 * time 2016/2/14
 * 
 * */

package model;

public class count_score {
	
	private int score_id;
	
	private int match_id;
	
	private int desk_id;
	
	private int score_cardid;
	
	private String score_declarationman;
	
	private String score_declaration;
	
	private String score_firstcardid;
	
	private int score_solution;
	
	private int score_nssolution;
	
	private int score_ewsolution;

	/**
	 * @return the score_id
	 */
	public int getScore_id() {
		return score_id;
	}

	/**
	 * @param score_id the score_id to set
	 */
	public void setScore_id(int score_id) {
		this.score_id = score_id;
	}

	/**
	 * @return the match_id
	 */
	public int getMatch_id() {
		return match_id;
	}

	/**
	 * @param match_id the match_id to set
	 */
	public void setMatch_id(int match_id) {
		this.match_id = match_id;
	}

	/**
	 * @return the desk_id
	 */
	public int getDesk_id() {
		return desk_id;
	}

	/**
	 * @param desk_id the desk_id to set
	 */
	public void setDesk_id(int desk_id) {
		this.desk_id = desk_id;
	}

	/**
	 * @return the score_cardid
	 */
	public int getScore_cardid() {
		return score_cardid;
	}

	/**
	 * @param score_cardid the score_cardid to set
	 */
	public void setScore_cardid(int score_cardid) {
		this.score_cardid = score_cardid;
	}

	/**
	 * @return the score_declarationman
	 */
	public String getScore_declarationman() {
		return score_declarationman;
	}

	/**
	 * @param score_declarationman the score_declarationman to set
	 */
	public void setScore_declarationman(String score_declarationman) {
		this.score_declarationman = score_declarationman;
	}

	/**
	 * @return the score_declaration
	 */
	public String getScore_declaration() {
		return score_declaration;
	}

	/**
	 * @param score_declaration the score_declaration to set
	 */
	public void setScore_declaration(String score_declaration) {
		this.score_declaration = score_declaration;
	}

	/**
	 * @return the score_firstcardid
	 */
	public String getScore_firstcardid() {
		return score_firstcardid;
	}

	/**
	 * @param score_firstcardid the score_firstcardid to set
	 */
	public void setScore_firstcardid(String score_firstcardid) {
		this.score_firstcardid = score_firstcardid;
	}

	/**
	 * @return the score_nssolution
	 */
	public int getScore_nssolution() {
		return score_nssolution;
	}

	/**
	 * @param score_nssolution the score_nssolution to set
	 */
	public void setScore_nssolution(int score_nssolution) {
		this.score_nssolution = score_nssolution;
	}

	/**
	 * @return the score_ewsolution
	 */
	public int getScore_ewsolution() {
		return score_ewsolution;
	}

	/**
	 * @param score_ewsolution the score_ewsolution to set
	 */
	public void setScore_ewsolution(int score_ewsolution) {
		this.score_ewsolution = score_ewsolution;
	}

	/**
	 * @return the score_solution
	 */
	public int getScore_solution() {
		return score_solution;
	}

	/**
	 * @param score_solution the score_solution to set
	 */
	public void setScore_solution(int score_solution) {
		this.score_solution = score_solution;
	}

}
